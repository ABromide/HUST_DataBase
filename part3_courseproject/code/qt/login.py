...
class login(QDialog):
	...
	def initUI(self):
		...
		# 打个招呼
		self.lb = QLabel('北京大兴国际机场机票预订管理系统',self)
		self.lb.setAlignment(Qt.AlignCenter)
		self.vbox.addWidget(self.lb)

		# 用户类型
		self.hbox1 = QHBoxLayout()
		self.rb1 = QRadioButton('用户',self)
		self.rb1.setChecked(True)
		self.rb2 = QRadioButton('管理员',self)
		self.hbox1.addWidget(self.rb1)
		self.hbox1.addWidget(self.rb2)
	...
	def confirm(self):
		...
		if [self.ct1,self.ct2] in roots and self.rb2.isChecked(): #管理员
			self.hide()
			self.rw = root_window()
			self.rw.show()
		elif [self.ct1,self.ct2] in users and self.rb1.isChecked(): #用户
			self.hide()
