...
class InputInfo(QMainWindow):
	...
	def initUI(self):
		...
		self.setGeometry(200,300,370,360)
...
class GetTicket_Bill(QMainWindow):
	...
	def initUI(self):
		...
		self.setGeometry(200,300,620,400)
		self.setFixedSize(self.width(),self.height())
		...
		self.pb1 = QPushButton('返回查询界面',self)
...
class AirSearch(QMainWindow):
	...
	def initUI(self):
		...
		#更改尺寸
		self.setFont(QFont("Arial",12))
		self.setGeometry(200,300,1600,600)
		self.setFixedSize(self.width(),self.height())
		...
		self.order_menu.addAction(self.back_action)
	...
	def GetSeat(self,info,tag):
		...
		# 直达
		if tag == 0:
			data = re.compile(r'\d+').search(data).group(0) # 注意 match必须从头开始，search则没这个要求
		...
		# 中转
			...
			index2 = data.find('1',int(len(data)/4*3))
	...
	def about_sys(self):
		...
		关于系统
		'''
		QMessageBox.about(self,'关于系统','北京大兴机场机票预订管理系统\n版本：Version 1.0')
