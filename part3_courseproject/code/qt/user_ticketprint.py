...
class ticketprint(QMainWindow):
	...
	def initUI(self):
		...
		self.setWindowTitle('北京大兴国际机场机票')

		# 标志
		pic = QPixmap('ticket.png')
		self.lb1 = QLabel(self)
		self.lb1.setGeometry(50,20,400,90)
		...
		self.pb1.setGeometry(300,320,100,50)
		self.pb1.clicked.connect(self.Print)

		# 返回
		self.pb2 = QPushButton('返回',self)
		self.pb2.setGeometry(600,320,100,50)
	...
	def Print(self):
		...
		pix = screen.grabWindow(self.winId(),0,0,1000,300)
...
class user_ticketprint(QMainWindow):
	'''
	打印机票页面
	'''
	def __init__(self):
	...
	def query(self):
		...
		sql = 'SELECT InfoID FROM GetTicketInfo WHERE InfoID = ' + self.le1.text() + ' AND token = "' +\
			self.le2.text() + '" AND PickDate = "' + self.date + '";'
